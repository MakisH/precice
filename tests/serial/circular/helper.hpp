...
inline void cyclicExplicit(TestContext &context)
{
  ...
  participant.setMeshVertices(meshName, coords, ids);

  std::vector<double> data{0, 0};

  participant.writeData(meshName, wid, ids, data);
  participant.initialize();

  BOOST_TEST_CONTEXT("first step")
  {
    BOOST_TEST_REQUIRE(participant.isCouplingOngoing());
    double preciceDt = participant.getMaxTimeStepSize();
    participant.readData(meshName, rid, ids, preciceDt, data);
