...
BOOST_AUTO_TEST_CASE(ExportTimeseries)
{
  ...
  BOOST_REQUIRE(participant.getMeshDimensions(meshName) == 3);
  ...
  if (context.isNamed("ExporterOne")) {
    ...
    participant.setMeshVertices(meshName, {coordsPtr, coordsSize}, vertexIds);
  }
  ...
  if (context.isNamed("ExporterOne")) {
    ...
    while (participant.isCouplingOngoing()) {
      ...
      participant.writeData(meshName, sdataName, vertexIds, sdata);
      participant.writeData(meshName, vdataName, vertexIds, vdata);
